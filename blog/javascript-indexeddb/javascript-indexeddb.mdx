...
description: "A complete tutorial for IndexedDB, a master class to cover every aspect. We will build a TODO App to cover all aspects of IndexedDB in a step-by-step manner."
date: "2023-04-25"
...
1. **IndexedDB databases store key-value pairs:** The values can be complex structured objects, and keys can be properties of those objects.
1. **IndexedDB is built on a transactional database model:** Everything you do in IndexedDB always happens in the context of a transaction.
1. **Asynchronous APIs:** The IndexedDB API is mostly asynchronous or event-based.
1. **Built on a transactional database model:** IndexedDB is built on a transactional database model. Everything you do in IndexedDB always happens in the context of a transaction.
1. **Dom Events:** IndexedDB uses DOM events to notify you when results are available. DOM events always have a type property
1. **Object Oriented, Not Relational:** IndexedDB is object-oriented. IndexedDB is not a relational database with tables representing collections of rows and columns. This fundamental difference affects the way you design and build your applications.
1. **High Capacity:** In GBs, much more than local or session storage
